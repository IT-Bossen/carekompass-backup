...
| 2026-05-28 | 21 | **Auto-apply migrationer via GitHub Action.** Lovable byggde `.github/workflows/migration-drift-check.yml` — push av `supabase/migrations/*.sql` till main triggar workflow:n som applicerar pending migreringar atomiskt mot Lovable Cloud (varje migration i en transaktion). PR gör dry-run + kommentar; merge applicerar. Failed → GitHub-issue automatiskt. **Override:** ersätter manuell `bunx supabase db push` + dashboard-deploy från `02 §12` / `06 §4.2`. Konsekvens: destruktiva ändringar oåterkalleliga post-merge → **branch protection på main är nu obligatoriskt** (06 §4.5). | 02 §12, 06 §4, .github/workflows/migration-drift-check.yml |
| 2026-05-28 | 21b | **Lovable-sandbox-drift incident.** Commit `16ccc8f` ("Enade secret till SUPABASE_DB_URL") raderade oavsiktligt design/ + docs/10 + agent docs/10-referenser eftersom Lovables sandbox-filsystem inte var synkat med commit `6e0390d` när Lovable gjorde sin nästa edit. Lovables full-snapshot-push överskrev tyst tillbaka det Claude Code committat strax innan. Återställt via `3bb47c8`. **Förebyggande:** branch protection på main krävs så Lovables sandbox-push får conflict istället för silent overwrite. Tills branch protection är på plats: ge Lovable tid att sync:a mellan Claude Code-pushar (öppna projektet i Lovable, vänta tills "uppdaterar"-indikatorn försvinner, innan ny edit). | 06 §4.5; commits 16ccc8f, 6e0390d, 3bb47c8 |
