...
- **`docs/10-design-spec.md`** — **the visual sanning + translation contract**. Especially: typography (`§3` — Inter + Newsreader + JetBrains Mono + the type scale), color tokens with v4-style `--ck-*` names + **shadcn translation table (`§6`)** (`ck-btn` → `Button`, `ck-card` → `Card`, `--ck-bg` → `--background`, etc.), component patterns (`§5`), desktop layouts + AppShell (`§7`), mobile conventions (`§8` — bottom tab bar + center FAB, sticky action bar, 44–56px primary touch-targets, list+chevron), **screen → code-target traceability (`§10`)**, mock-data vs. domain-truth (`§12`), and the 9 open design decisions Toni owns (`§14`).
- **`design/`** at repo root — the design artifact itself (React canvas with `index.html` + `components/<area>.jsx` + `styles.css`). **Reference material**, not production code; you implement from `docs/10`'s patterns + the source files as visual reference. The `tweaks-panel.jsx` is a designer's brand-color experimenter, not a feature.
