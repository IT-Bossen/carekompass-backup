...
**Read `CLAUDE.md` and the relevant `docs/` (`05-lex-maria…`, `06-consent-egenkontroll…`,
`08-gdpr-and-scope`)** so you judge against the intended, regulated behavior — while remembering
the docs are aspirational and the code is the source of truth.
...
- On the **code/diff**: do the migrations, edge functions, hooks, and components actually enforce those rules?
...
- **Multi-tenant isolation integrity.** Operational data (deviations, risks, ordinations, medications, hygiene) is **clinic-scoped** — a user in Clinic A must never read/write Clinic B's rows. Company-scoped data (customers, policy documents, audit) stays within the company; tenant-shared data (customer profiles, when `tenants.shared_customers = true`) only crosses companies when opt-in is on. Verify reads filter via `get_user_clinic_ids`/`get_user_company_ids` and writes set the correct `company_id`/`clinic_id`. Flag any query that assumes a single clinic/company or leaks across a ring.
- **Module / plan gating.** A module's UI (`ModuleGuard moduleKey=…`) and its `is_module_enabled` / `company_features` flag agree, and the feature-key mapping (`policies→module_documents`, `deviations→module_deviations`, `risks→module_risk`, …) is correct. Disabled or `read_only`-plan companies can't write.
- **Audit & immutability.** Mutating workflows write `audit_logs` (with a `request_id`); `audit_logs` and version/acknowledgment tables are never updated or deleted (deny policies). Audit is retained, not erased, even during GDPR deletion — personal data is anonymized instead.
- **GDPR & consent.** Consent is logged (`gdpr_consent_logs`, with `consent_type`); erasure/anonymization respects company-vs-tenant scope and never deletes audit history.
...
- **Swedish domain terminology.** User-facing copy uses correct, consistent Swedish (Avvikelse, Styrdokument/Policydokument, Riskhantering, Kundregister, Klinik, Bolag, Hygien, Ordination, Läkemedel, Compliance) and matches existing wording and the industry-specific labels from `useIndustryConfig().t()`. Identifiers stay English.
...
Read the spec and the changed files in full; cross-reference migrations + `types.ts` +
`usePermissions.ts` + the guards to confirm rules line up across layers. Use `Bash` for read-only
