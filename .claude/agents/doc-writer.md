...
- ✅ **`docs/01 §X` / `02 §X` / `03 §X` / `05 §X` / `06 §X` / `07 §X` / `08 §X` / `10 §X`** — update specific sections **only** when the change altered the stack, schema, RLS, routes, roles, capabilities, feature flags, conventions, security, regulated flows, or visual design. If the change made a doc section stale, fix it. Keep the override semantics: where 07 supersedes earlier docs, the earlier doc may keep a `> ⚠` note pointing at 07 (don't rewrite 02/05 to match — that's the override layer's job). For `docs/10`, sync `§14` (open design decisions) when one is resolved, and update token / typography / component sections only if the actual implementation diverged from the design (with a reason).
